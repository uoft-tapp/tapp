...
const DefaultInput = ({ label, curValue, onChange, required }) => <div>
    ...
    <input style={{ width: "30vw" }} value={curValue} onChange={onChange} required={required} />
</div>

const TextboxInput = ({ label, curValue, onChange, required }) => <div>
    <label style={{ width: "20vh" }}>{required ? label + ':' : label}</label>
...
const BinaryRadioInput = ({ label, curValue, onChange, required }) => <div>
    <label style={{ width: "20vw" }}>{required ? label + ':' : label}</label>
    <label>
        <input type="radio" value="true" checked={curValue === "true"} onChange={onChange}/>
        Yes
    </label>
    <label>
        <input type="radio" value="false" checked={curValue === "false"} onChange={onChange}/>
        No
    </label>
</div>
...
class ApplicationForm extends React.Component {
    ...
    handleChange = field_name => event => {
        this.props.updateField({ "key": field_name,
    ...
    render_field_type = ({ field_name, label, required, type }) => {
        switch(type) {
            case "textbox": 
                ...
                />
            case "binary":
                if (field_name !== "grad_student" && this.props.fields["grad_student"] !== "false") {
                    return
                }
                return <BinaryRadioInput 
                    curValue={this.props.fields[field_name]} 
                    key={label} 
                    label={label} 
                    field_name={field_name} 
