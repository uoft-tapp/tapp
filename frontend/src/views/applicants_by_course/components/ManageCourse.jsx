...
import { changeInstructorPref, selectApplicant, removeApplicant } from '../../../api/actions/applicants_by_course';
import { Button, Card, DropdownButton, Dropdown } from 'react-bootstrap';
import ReactTable from 'react-table';

const isAdmin = true;

const SelectButton = connect(null, { selectApplicant })(({ selectApplicant, positionId, applicantId }) => (
	<Button
...
const RemoveButton = connect(null, { removeApplicant })(({ removeApplicant, positionId, applicantId }) => (
	<Button
		className="full-width"
		variant="outline-secondary"
		size="sm"
...
const demoteColumn = {
	...
	Cell: ({ original: { applicantId, positionId, locked } }) => {
		if (locked) {
			return (
				<Button className="full-width" variant="outline-secondary" size="sm" disabled>
...
const InstructorPrefDropdown = connect(null, {
	changeInstructorPref
})(({ changeInstructorPref, applicantId, positionId, instructorPref, title }) => (
...
const instructorPrefColumn = {
	...
	filterMethod: (filter, row) => {
		return filter.value === 'all' || parseInt(filter.value, 10) === row['instructor_pref'];
	},
	Filter: ({ filter, onChange }) => (
		...
		>
			...
			{instructorPrefOptions.map((option) => <option value={option.value}>{option.label}</option>)}
		</select>
	)
};
...
const getTableCols = (permissions) => {
	...
	return isAdmin ? (isSelectedTable ? adminSelectedCols : adminAvailableCols) : instructorCols;
};
	...
	render() {
		...
		return (
			<div>
				<Card>
					<Card.Header>
						{this.props.position ? this.props.position.course_code : 'Course Name Unavailable'}
					</Card.Header>
					<Card.Body>
						<h3>Selected</h3>
						<ReactTable
...
export default connect(({ ui: { applicants, positions: { list } } }, { positionId }) => ({
